...
bool ATLOW_WaitNoExpect(uint32_t waitTime, char *pResp) {
  ...
  uint8_t *pRecvChar;
  uint32_t baseWaitTime = SYSTMR_millis();
  uint32_t baseWaitByte = SYSTMR_millis();

  pRecvChar = (pResp) ? ((uint8_t *)pResp) : ((uint8_t *)spResp);

  firstRecv = false;
  while (SYSTMR_millis() - baseWaitTime < waitTime) {
    if (firstRecv) {
      if (SYSTMR_millis() - baseWaitByte > TIMEOUT_INCOMING_BYTE * 1000) {
        *pRecvChar = '\0';
        return true;
      }
    }
...
bool ATLOW_Wait(uint32_t timeOut, char *pWaitStr, char *pResp) {
  ...
  while (SYSTMR_millis() - baseWaitTime < timeOut) {
    if (firstRecv) {
      if (SYSTMR_millis() - baseWaitByte > TIMEOUT_INCOMING_BYTE * 1000) {
        *pRecvChar = '\0';
        return false;
